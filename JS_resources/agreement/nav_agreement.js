...
Navicon.nav_agreement = (function () {
   ...
   var contactOrAutoOnChange = function (context) {
      ...
      if (contactAttr && creditidAttr && creditidControl && autoAttr) {
   ...
   var usedSetSumma = function (autoid) {
      var summaAttr = baseUtils.getAttribute("nav_summa");
      if (summaAttr && autoid) {
         var promiseAutoSumm = Xrm.WebApi.retrieveRecord(
            "nav_auto",
            `${autoid}`,
            "?$select=nav_amount"
         );
   ...
   var creditOnChange = function (context) {
      ...
      if (creditAttr && summaControl && dateAttr) {
         ...
         if (creditAttr.getValue()) {
            ...
            if (dateAttr.getValue()) checkCreditValidity();
