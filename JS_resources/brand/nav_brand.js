...
function createDatatable (data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        elemA0.setAttribute('onClick', `parent.Xrm.Navigation.openUrl("https://` + window.location.host + "/main.aspx?app=d365default&forceUCI=1&pagetype=entityrecord&etn=nav_credit&id=" + data[i].nav_credit_creditid + `")`);
        ...
        elemA1.setAttribute('onClick', `parent.Xrm.Navigation.openUrl("https://` + window.location.host + "/main.aspx?app=d365default&forceUCI=1&pagetype=entityrecord&etn=nav_model&id=" + data[i].nav_modelid + `")`);
        elemA1.appendChild(document.createTextNode(data[i].nav_name));

        td0.appendChild(elemA0);
        td1.appendChild(elemA1);
        td2.appendChild(document.createTextNode(data[i].nav_credit_creditperiod));
